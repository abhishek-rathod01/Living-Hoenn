...
## 8. Phase 5 groundwork — trainer identity (verified)

- `struct Trainer` (ROM `gTrainers` table): **sizeof 0x28**, trainerClass @+0x01,
  trainerName @+0x04 (11 bytes, 0xFF-terminated), partySize @+0x20, party ptr
  @+0x24 — proven by 32-bit compile AND matching the header's own offset
  comments. `lua/trainer_info.lua` reads class+name; tested by decoding a
  charmap-encoded name from simulated ROM.
- Match Call: text is composed into gStringVar4 (src/match_call.c), and
  `struct MatchCallState` holds the calling trainerId as a u16 at +4 —
  watch it flip 0 -> nonzero as the call trigger. Symbols to grep for this
  phase: `gTrainers`, `sMatchCallState`.
- Map names: `bridge/world_tables.py` MAPS[(group,num)] generated by joining
  map_groups.json -> each map.json's region_map_section ->
